...
define(['N/query', 'N/log', 'N/ui/serverWidget', 'N/runtime'], function(query, log, serverWidget, runtime){
  ...
  function onRequest(context){
    ...
    var cantidad   = context.request.method == 'POST' ? context.request.parameters.custpage_selectfield .split(delimiter) : '0,1,2,3,4,5,\'Todos\'';
...
if(cantidad== "todos"){
if(marca1!=""){
  wh="  join customrecord681 on N2.custitem15=customrecord681.id where customrecord681.id='"+marca1+"' and N2.dias_invent_monterrey  < 6 "
}else{ wh="where  N2.dias_invent_monterrey  < 6"}
...
if(marca1!=""){
wh="  join customrecord681 on N2.custitem15=customrecord681.id where customrecord681.id='"+marca1+"' and N2.dias_invent_monterrey  ="+cantidad+" "
}else{ wh="where  N2.dias_invent_monterrey  ="+cantidad+""}

}
var  wh2=" "

if(Clasificacion2!=""){
wh2="  and  N2.custitem4 ='"+Clasificacion2+"' "
}else{ wh2=""}
...
valor_dedia=(dias1432*2)-dias143222
  ...
  ventadias_Dias_m=   "ROUND ( (N1.stockDisponible)   /(SUM( "+
    ...
    var sqlPrincipal= "SELECT N2.* FROM ( "+
     ...
     ""+ventadias_Dias_m+""+
          "N1.quantityonorder AS quantityonorder,"+
              ...
              "       AND N1.status IN('D', 'A', 'B') "+
              "     THEN N1.stock - N1.cantidadRecibida "+
              "     ELSE 0 "+
              "     END "+
              "   )                 AS pendiente_X_Recibir, "+
   ...
   "   N1.location    AS  sucursal, "+
              ...
              "     inl.quantityOnHand   AS stockDisponible, "+
        ...
        "     tra.terms      AS terms ,"+   
              ...
              "   FROM Item AS itm "+
              "     LEFT JOIN  TransactionLine AS trl ON trl.Item = itm.ID "+
...
valor_dedia=(dias1432*2)-dias143222
    ...
    var dias2222=dias1432+1
        ...
        innerHtml+= '<br/><br/><input type="text" id="valor_Dias" name="Dias" class="form-control" value="'+Clasificacion2+' Dias laborales de diferencia entre las fechas" readonly=""/>';
    ...
    if ( records.length > 0 ) {
      ...
      for ( i = 1; i < columnNames.length; i++ ) {
        ...
        if (columnNames[i].indexOf("fecha_final_mes_f")   !== -1) continue;
            if (columnNames[i].indexOf("pendiente_x_recibir") !== -1)continue;
...
if (columnNames[i].indexOf("dias_invent_monterrey")   !== -1) continue;
if (columnNames[i].indexOf("comprador1")   !== -1) continue;

        innerHtml+= '     <th>'+columnNames[i]+'</th>';
      }
    

        innerHtml+= '         <th>Dias Inventario</th>';
...
var variable_c_final_2=0;
 var dias_inventario =0;
  var dias_invent_monterrey =0;
    ...
    if (column.indexOf("pendiente_x_recibir") !== -1)continue;
...
if (column.indexOf("quantitybackordered") !== -1)continue;
        ...
        if (column.indexOf("custitem4") !== -1)continue;
         ...
         else if (column.indexOf("dias_invent_monterrey") !== -1)
            dias_invent_monterrey = parseFloat(value);
            else if (column.indexOf("comprador1") !== -1)
            comprador1 = value;
          ...
          innerHtml += '    <td align="'+ (es_numero ? "right" : "right") +'" class="precio">' + (es_numero ? parseFloat(value).toFixed(1) :  parseFloat(value).toFixed(1)) + '</td>';
...
else if(custitem4=="2"){// valor 2= b
  variable_a=45
  variable_text_aa="B"
}
else if(custitem4=="3"){// valor 3 = c
  variable_a=30
  variable_text_aa="C"
}
else{variable_text_aa="S/C"}



variable_b=(dias_inventario.toFixed(2)*variable_a)-stock 
if(variable_b<=0){
 ...
 if   (comprador1==null){comprador1='Sin asignar';}
if (isNaN(variable_c_final)) { variable_c_final_2=0}else{variable_c_final_2=quantityonorder}
if (Venta_con_stock_actual>150){Venta_con_stock_actual='150'}else{Venta_con_stock_actual=Venta_con_stock_actual.toFixed(0)}

            innerHtml += '        <td align="right" class="diasinv">'+parseInt(dias_invent_monterrey)     +'</td>';


      

        innerHtml += '        <td align="right">'+variable_text_aa     +' </td>';
        innerHtml += '        <td align="right">'+pendiente_x_recibir     +' </td>';
        innerHtml += '        <td align="right">'+comprador1     +' </td>';
...
html += records > 0 ? '   "order": [[3, "des"], [11, "asc"], [10, "asc"]],' : '';
...
html += ');';
 ...
 html+="$('#resultsTable tr').each(function() {"
        html+="$(this).find('td:eq(5), th:eq(5)').hide(); "
   ...
   html += ' precios.forEach(function(precio) {';
     html += ' var valor = parseFloat(precio.textContent);';
        ...
        html += '  precio.textContent = valor.toLocaleString("en-US", { style: "currency", currency: "USD" });';
           ...
           html += '   if(prop==3){'
           html += '             col3+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==4){'
           html += '             col4+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==5){'
           html += '             col5+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==6){'
           html += '             col6+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==7){'
           html += '             col7+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==8){'
           html += '             col88+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==9){'
           html += '             col9+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==10){'
           html += '             col10+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==11){'
           html += '             col11+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==12){'
           html += '             col12+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '  else if(prop==13){'
           html += '             col13+=parseFloat(value[prop]); '; 
           html += '          }';
           html += '          }';
           html += '    }); ';
         ...
         html += '    col3 = col3.toLocaleString("en-US", { style: "currency", currency: "USD" });';
           ...
           html += '$("#ttf").empty(); $("#ttf").append( `<tr><td  ></td><td  class="text-right "> </td>  <td  class="text-right "> </td>  <td  class="text-right ">`+col3+` </td>  <td  class="text-right ">`+parseInt(col4)+` </td>  <td  class="text-right ">`+parseInt(col5)+` </td>  <td  class="text-right ">`+parseInt(col6)+` </td>  <td  class="text-right "></td>  <td  class="text-right "> </td>  </tr>` ); ';  
