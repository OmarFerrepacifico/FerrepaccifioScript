...
var sqlPrincipal = "SELECT t1.displayname as codigo,t1.articulo,\
t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2024' AÑO  , '0' MES FROM\
         (\
             SELECT \
  ...
  inventoryitemlocations.location IN("+location+") AND item.isInactive='F' and  item.id in ("+articulo+")   GROUP BY item.itemid,CUSTOMRECORD671.name,item.displayname ORDER BY valor_de_stock DESC\
         ) t1 LEFT JOIN(\
             SELECT\
    ...
    SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
     AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
      ...
      ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
         ...
         )t2 ON t2.articulo=t1.articulo WHERE (cantidad_disponible!='' OR cantidad_disponible IS NOT NULL)  AND (valor_de_stock!='' OR valor_de_stock IS NOT NULL) ";
             }
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           )t2 ON t2.articulo=t1.articulo WHERE (cantidad_disponible!='' OR cantidad_disponible IS NOT NULL)  AND (valor_de_stock!='' OR valor_de_stock IS NOT NULL)  union all "+
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2024' AÑO  , '3' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
                    ...
                    SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
                     AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
                      ...
                      ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
                         ...
                         "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2024' AÑO  , '2' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2024' AÑO  , '1' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
                    ...
                    SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
                     AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
                      ...
                      ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
                         ...
                         "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '12' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '11' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '10' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
                    ...
                    SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
                     AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
                      ...
                      ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
                         ...
                         "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '9' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '8' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '7' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '6' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '5' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
      ...
      SUM(  SOLine.quantity )*-1 as cantidad FROM Transaction AS SalesOrder INNER JOIN TransactionLine AS SOLine ON ( SOLine.Transaction = SalesOrder.ID ) AND ( SOLine.MainLine = 'F' ) INNER JOIN location\
       AS Ubicacion ON ( SOLine.location= Ubicacion.id ) INNER JOIN Item ON ( Item.ID = SOLine.Item ) LEFT JOIN itemLocationConfiguration ON\
        ...
        ( SalesOrder.custbody_bex_tiponc=1 OR SalesOrder.custbody_bex_tiponc=3) AND (ubicacion.id IN ("+location+")) GROUP BY item.itemid\
           ...
           "SELECT t1.displayname as codigo,t1.articulo,\
  t2.cantidad_vendida,venta,utilidad utilidad_en_pesos,round((utilidad/venta)*100) utilidad_porcentaje, '2023' AÑO  , '4' MES FROM\
           (\
               SELECT \
                   item.displayname,item.itemid as articulo,SUM( inventoryitemlocations.quantityavailable) as cantidad_disponible,\
     SUM( inventoryitemlocations.quantityavailable*inventoryitemlocations.averagecostmli) as valor_de_stock,CUSTOMRECORD671.name as proveedor\
...
html += '$("#ttf").empty(); $("#ttf").append( `<tr><td  ></td><td  class="text-right ">`+col1+` </td>  <td  class="text-right ">`+col2+` </td>  <td  class="text-right ">`+col3+` </td> <td  class="text-right ">`+col4+` </td>  <td  class="text-right ">`+porcent1+`% </td>  </tr>` ); ';  
html += '  });'
...
html += 'var columnData = table.column(6).data();'

// Recorrer los datos y eliminar los puntos decimales y ceros
html += 'columnData.each(function (value, index) {'
  // Convertir el valor a un número flotante y luego a un entero
  html += ' var intValue = parseInt(parseFloat(value));'
  // Establecer el nuevo valor en la celda
  html += 'table.cell(index, 6).data(intValue);'
  html += '});'
